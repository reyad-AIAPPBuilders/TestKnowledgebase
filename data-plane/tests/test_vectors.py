"""Tests for DELETE /api/v1/vectors/{source_id} and PUT /api/v1/vectors/update-acl."""
...
# ── DELETE /vectors/{source_id} ──────────────────────────────────────


def test_delete_vectors_success(client, mock_qdrant):
    mock_qdrant.delete_by_source_id.return_value = 47

    response = client.delete("/api/v1/vectors/doc_abc123?collection_name=wiener-neudorf")
...
def test_delete_vectors_request_id(client, mock_qdrant):
    ...
    response = client.delete(
        "/api/v1/vectors/doc_xyz?collection_name=wiener-neudorf",
