...
class OnlineIngestMetadata(BaseModel):
    ...
    department: str | None = Field(None, description="Department within the organization")


class OnlineIngestRequest(BaseModel):
    """Request to ingest web-scraped content into the vector database.

    Takes scraped/parsed text + ACL and runs:
    ...
    url: str = Field(..., description="Source URL (stored as metadata)")
